...
<blockquote>
  本章节详细介绍基于数据模型处理器和Gou Query
  DSL两种数据查询方式。建议在阅读前学习关系型数据库基础知识，了解SQL语法
  ，可以编写基本的SQL语句。
</blockquote>
...
<Extend
  title="通过 URL Query String 传递查询参数"
  desc="了解如何与 API 绑定，通过 URL Query String 传递查询参数"
  link="d.参考手册/c.接口#输入参数"
></Extend>

## Gou Query DSL

Gou Query DSL 可以在数据流中使用，一般用于数据分析统计等更为复杂的场景。
...
<Notice type="success">
  提示：在数据流中通过 <strong>&#123;&#123;$xxx&#125;&#125;</strong> 或
  <strong> ?:$xxx </strong>
  使用变量写法等效。
</Notice>

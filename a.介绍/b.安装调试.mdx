...
3. 安装 Docker [下载安装 Docker](https://docs.docker.com/get-docker/)

4. 创建并启动容器:`docker run -d --name yao -v <project root>:/data/app -p 5099:5099 yaoapp/yao-<CPU>:latest`
...
<Notice type='success'>
	...
	MySQL、PostgreSQL数据库作为数据源，请参照数据库官方文档下载并安装。推荐使用 MySQL 8
	或Postgres14。
</Notice>
...
<Extend title='环境变量文档' desc='查阅全部环境变量参考文档' link='d.API参考/l.环境变量'></Extend>

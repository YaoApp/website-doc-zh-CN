...
2. 创建并启动容器: `docker run -d --name yao -v <project root>:/data/app -p 5099:5099 yaoapp/yao:0.9.1-amd64-dev`
...
| 镜像                | 环境     | 使用场景                                                                  |
| ------------------- | -------- | ------------------------------------------------------------------------- |
| yao:0.9.1-amd64-dev | 开发环境 | `amd64` 适用于 MacOS Intel 使用这个镜像, Windows (X86 64 位), Linux 64 位 |
| yao:0.9.1-arm64-dev | 开发环境 | `arm64` 适用于 MacOS M1, 树莓派,RK 开发板等使用这个镜像。                 |
| yao:0.9.1-amd64     | 生产环境 | 使用云部署                                                                |
| yao:0.9.1-arm64     | 生产环境 | 适用于边缘设备部署                                                        |

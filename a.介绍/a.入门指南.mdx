...
我们准备了一个在线开发调试环境，帮助您快速上手体验。**在线开发环境仅做体验使用， 数据每小时重置** , 在实际开发中，请在本地安装 Yao 开发调试，具体参考 [安装调试](/a.介绍/b.安装调试.mdx) 章节的文档。如果您的应用开发完成，需要在生产环境部署，请参考 [部署](/e.部署.mdx) 章节文档。
...
<Notice type="warning">
  ...
  JSON 调试界面的过程当中遇到界面为空的情况。
  [详细说明](/components/a.%E9%A1%B5%E9%9D%A2%E6%8F%8F%E8%BF%B0/0.%E6%B3%A8%E6%84%8F%E4%BA%8B%E9%A1%B9)
</Notice>

<Notice type="info">
  调试过程中，可以使用 `yao run` 命令查询数据。例如: 在命令行终端中运行 `yao run
  models.customer.find 1` 即可查询 `customer` 数据表 ID=1 的数据。
</Notice>

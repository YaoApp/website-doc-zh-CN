...
`http://<IP|域名>:<YAO_PORT>/<adminRoot>/x/Form/<Widget ID>/view`

编辑模式:

`http://<IP|域名>:<YAO_PORT>/<adminRoot>/x/Form/<Widget ID>/edit`

<Detail title="查看表单示例 /data/app/forms/pet.tab.json">
  ...
  "sections": [
    {
      ...
      "columns": [
        { "name": "表格", "width": 24 }, // name: fiields.form.<KEY> ,  width 字段显示宽度, 24栅格
        {
          "width": 24,
          ...
          "tabs": [
            {
              "title": "Base", // 标签名称
              "columns": [
                { "name": "ID", "width": 12 }, // name: fiields.form.<KEY> ,  width 字段显示宽度, 24栅格
                { "name": "名称", "width": 12 }
              ]
            },
            {

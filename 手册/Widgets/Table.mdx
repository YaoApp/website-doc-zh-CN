...
<blockquote>
  Table Widget 用于描述表格界面，可以用来制作数据列表界面。 Table DSL 文件放在
  tables 目录中, 文件扩展名为 .tab.json。
</blockquote>
...
DSL 文件名由 **小写字母**, **下划线 “\_”**, **横杠 "-"** 组成, 必须由 **小写字母** 开头, Widget ID 为文件名称(不含扩展名), 如包含多级目录用 **"."** 替代 **"\/"**
...
| layout | Object | 是     |        | [查看详情](#layout-object) | 表格界面布局。显示字段、筛选器、批量编辑等                                    |
| fields | Object | 是     |        | [查看详情](#fields-object) | 表格字段定义。指定表格列字段, 表格筛选器字段定义                              |
| config | Object | 是     |        | [查看详情](#config-object) | 表格界面配置项。表格满屏显示等配置                                            |
...
```jsonc
{
  "名称": {
    ...
    "edit": {
      ...
      "compute": "scirpts.pet.FormatName",
...
| 处理器                | 参数表                                     | 返回值                     | 说明                                                           |
| --------------------- | ------------------------------------------ | -------------------------- | -------------------------------------------------------------- |
| yao.table.Setting     | `[]`                                       | 返回表格配置               | 返回表格 DSL 信息                                              |
| yao.table.Xgen        | `[]`                                       | 返回表格配置               | 返回表格配置信息, 用于 XGEN 界面引擎页面渲染                   |
| yao.table.Search      | `[<查询参数>, <当前页码>, <每页显示记录>]` | 返回带有分页信息和记录数组 | 调用关联处理器，查询表格数据列表                               |
| yao.table.Get         | `[<查询参数>]`                             | 返回记录数组               | 调用关联处理器，查询表格数据列表                               |
| yao.table.Find        | `[<主键>,<查询参数>]`                      | 返回记录                   | 调用关联处理器，按主键查询表格数据详情                         |
| yao.table.Save        | `[<记录>]`                                 | 主键                       | 调用关联处理器，保存一条数据记录，参数包含主键则更新，否则创建 |
| yao.table.Create      | `[<记录>]`                                 | 主键                       | 调用关联处理器，创建数据记录                                   |
| yao.table.Delete      | `[<主键>]`                                 | 返回记录                   | 调用关联处理器，按主键删除记录                                 |
| yao.table.Insert      | `[<字段名称数组>, <二维记录值数组>]`       | 主键                       | 调用关联处理器，插入多条记录                                   |
| yao.table.Update      | `[<主键>,<记录>]`                          | 返回更新行                 | 调用关联处理器，更新单条记录                                   |
| yao.table.UpdateWhere | `[<查询参数>,<记录>]`                      | 返回更新行                 | 调用关联处理器，更新符合查询条件记录                           |
| yao.table.UpdateIn    | `[<主键数组>,<记录>]`                      | 返回更新行                 | 调用关联处理器，更新指定主键的数据记录                         |
| yao.table.DeleteWhere | `[<查询参数>]`                             | 返回更新行                 | 调用关联处理器，删除符合查询条件记录                           |
| yao.table.DeleteIn    | `[<主键数组>]`                             | 返回更新行                 | 调用关联处理器，删除指定主键的数据记录                         |

...
| 协议名称 | 文档地址                      |
| -------- | ----------------------------- |
| http     | [RESTFul API](#1-restful-api) |
| mqtt     | MQTT API **尚未实现**         |
| sock     | Socket API (内测中)           |
...
API 将通过路由 `/api/<group>/<path>`访问, 请求成功响应 `out` 中定义的状态码、Content-Type 和处理器(`process`)返回值， 请求失败响应异常状态码和 JSON 格式的错误消息 [查看异常规范](#4-异常规范)。
...
| in      | Array\<String\> | 请求参数表，将作为 `process` 的输入参数(`args`)。可以引用传入参数，可以为空数组 [查看输入参数](#3-输入参数)                            | 是     |

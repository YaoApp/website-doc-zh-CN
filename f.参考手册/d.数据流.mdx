...
数据流(`Flow`)用来编排数据查询逻辑，支持使用 JavaScript(ES5) 脚本处理各查询节点结果，可以作为处理 (`process`) 来使用，引用方式为 `flows.<数据流名称>` 。
...
| output      | Object              | 输出结果定义 [查看完整说明](#22-输出结果-output) | 是     |
...
一个数据流编排(`Flow`)可以有多个查询节点, 每个查询节点，可以调用一个处理器(`process`), 可以指定结果处理脚本和返回值，在查询节点用可以引用上下文信息，具体参阅相关文档: [输入和输出](#3-输入和输出) [上下文数据引用](#3-上下文数据引用) [结果处理脚本](#4-结果处理脚本)
...
| args    | Array<Any\>    | 处理器参数表.可以引用输入输出或上下文数据[查看完整说明](#3-上下文数据引用)                | 是     |
| script  | String         | 结果处理脚本，支持 ES5 [查看完整说明](#4-结果处理脚本)                                    | 否     |
...
可以根据业务需要，自由定义数据流(`Flow`)的输出结果。 可以使用 `{{$in}}` 引用数据流(`Flow`)调用时传入的参数、使用 `{{$res}}` 引用各个查询节点返回值。 [查看完整说明](#3-上下文数据引用)
...
查询节点支持使用 JavaScript 脚本(ES5)对处理器(`process`)返回值处理，`main()` 函数传入数据流(`Flow`)参数表、当前查询节点 process 返回值和查询节点返回值映射表。返回值可以使用 `$out` 变量引用。

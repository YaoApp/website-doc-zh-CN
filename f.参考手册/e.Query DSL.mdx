...
| `goods as g` | `数据表名称 as 别名`    | 在 `goods` 数据表中查询, 设置别名为 `g`, `g.字段名称` 与 `goods.字段名称` 等价 ([表达式文档](<(#2-字段表达式-Field-Expression)>)) |
...
- 如设置子查询 `query`([文档](#1.8-子查询-query))，解释器优先响应子查询逻辑，忽略 `from` 设定。 `query`和`from` 必填其一。
...
| 字段    | 类型                  | 说明                                                                                     | 必填项 |
| ------- | --------------------- | ---------------------------------------------------------------------------------------- | ------ |
| field   | Field Expression      | 字段表达式，不支持设置别名 `as` [查看字段表达式文档](<(#2-字段表达式-Field-Expression)>) | 是     |
...
| query   | Object QueryDSL       | 子查询；如设定 `query` 则忽略 `value` 数值。 [查看子查询文档](#1.8-子查询-query)         | 否     |
| comment | String                | 查询条件注释，用于帮助理解查询条件逻辑和在开发平台中呈现。                               | 否     |
...
| data-only | Bool | 仅在设定 `page` 或 `pagesize` 时有效。设定为 `true`, 查询结果为[数据记录数组 Array\<Object Record\>](#31-数据记录数组-arrayobject-record); 默认为 `false`, 查询结果为[带分页信息的数据记录数组 Object Paginate](#32-带分页信息的数据记录数组-Object-Paginate)。 | 否     |
...
`joins` 数据结构为 `Array<Object Join>` [查看数据结构](#Object-Join-数据结构)
...
| 字段    | 类型   | 说明                                                                    | 必填项 |
| ------- | ------ | ----------------------------------------------------------------------- | ------ |
| from    | String | 连接表名称，与 Object QueryDSL `from`相同。 [文档](#12-选择数据表-from) | 是     |
...
[查看 Object Record 数据结构](#3.3-数据记录数据结构-Object-Record)

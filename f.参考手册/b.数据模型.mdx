...
Yao 可以读取数据模型定义，实现数据迁移、元数据原子操作、元数据输入校验和元数据管理后台。元数据原子操作方法被映射为处理器(`process`)，支持模型间数据关系映射，可在业务逻辑（`Flow`）和业务接口(`API`)中访问查询。如采用`golang`语言开发业务插件(`Plugin`)，可以使用 Package `Gou` 访问模型的各个方法, 后续将提供 `NodeJS` 等语言 SDK。

## 1 命名规范

数据模型描述文件是以 **小写英文字母** 命名的 JSON 文本文件 `<name>.json`

| 文件夹 (相对应用模型根目录) | 文件名    | 模型名称             | Process (在 API /Flow 中引用)         |
| --------------------------- | --------- | -------------------- | ------------------------------------- |
| /                           | name.json | `name`               | `models.name.<process>`               |
| /group/                     | name.json | `gorup.name`         | `models.gorup.name.<process>`         |
| /group1/group2/             | name.json | `gorup1.gorup2.name` | `models.gorup1.group2.name.<process>` |

## 2 文档结构

数据模型定义文档，由基础信息、数据表、字段定义、索引定义、关系映射、默认数据和配置部分构成。[查看完整示例](#5-完整示例)
...
| 字段      | 类型                  | 说明                                            | 必填项 |
| --------- | --------------------- | ----------------------------------------------- | ------ |
| name      | String                | 模型中文名称 [查看完整说明](#21-基础信息)       | 是     |
| table     | Object                | 数据表定义 [查看完整说明](#22-数据表-table)     | 是     |
| columns   | Array\<Object\>       | 字段定义 [查看完整说明](#23-字段定义-columns)   | 是     |
| indexes   | Array\<Object\>       | 索引定义 [查看完整说明](#24-索引定义-indexes)   | 是     |
| relations | \[key:String\]:Object | 关系映射 [查看完整说明](#25-关系映射-relations) | 否     |
| values    | Array\<Object\>       | 默认数据 [查看完整说明](#26-默认数据-values)    | 否     |
| option    | Object                | 配置选型 [查看完整说明](#27-配置选项-option)    | 否     |
...
| type        | String                 | 字段类型， [查看完整说明](#字段类型)                                                        | 是     |
...
| length      | Integer                | 字段长度，对 `string` 等类型字段有效 [查看完整说明](#字段类型)                              | 否     |
| precision   | Integer                | 字段位数(含小数位)，对 `float`、`decimal` 等类型字段有效 [查看完整说明](#字段类型)          | 否     |
| scale       | Integer                | 字段小数位位数，对 `float`、`decimal` 等类型字段有效 [查看完整说明](#字段类型)              | 否     |
| option      | Array\<String\>        | 字段许可值，对 `enum` 类型字段有效 [查看完整说明](#字段类型)                                | 否     |
| default     | String\|Integer\|Float | 字段默认值                                                                                  | 否     |
| default_raw | String                 | 字段默认值，支持数据库函数，如 `NOW()` default 和 default_raw 同时存在 default_raw 优先级高 | 否     |
| crypt       | String                 | 字段加密存储方式(MySQL Only)。许可值 `AES`, `PASSWORD` [查看完整说明](#加密方式)            | 否     |
...
| validations | Array\<Object\>        | 字段校验规则 [查看完整说明](#校验方法)                                                      | 否     |
...
| \_\_tracking_id  | 用于追溯用户操作记录         |
| \_\_permission   | 用于定义字段权限             |
...
| 映射关系名称 | 关系   | 说明                                                               |
| ------------ | ------ | ------------------------------------------------------------------ |
| `hasOne`     | 一对一 | 模型 A 与模型 B 通过一对一关联 [查看完整说明](#251-hasone-一对一)  |
| `hasMany`    | 一对多 | 模型 A 与模型 B 通过一对多关联 [查看完整说明](#253-hasmany-一对多) |
...
| query   | Object QueryParam        | 关系查询参数默认值。如在查询时未指定关联查询参数，则替使用在模型中定义的查询参数 [查看完整说明](#3-查询参数-queryparam) | 否     |
...
查询条件可以通过 URL Query String 传入，[详见对照表](../api/protocol/http.md#url-query-string-与-queryparam-对照表)

## 4 处理器(`process`)

数据模型提供一组原子操作处理器 `process` , 这些处理器可用于服务接口(`API`)和业务逻辑(`Flow`)编排。

| 处理器              | 引用方式                            | 说明                                                                                                                          |
| ------------------- | ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| find                | models.模型名称.Find                | 查询单条记录 [详情](process/Find.md)                                                                                          |
| get                 | models.模型名称.Get                 | 按条件查询, 不分页 [详情](process/Get.md)                                                                                     |
| paginate            | models.模型名称.Paginate            | 按条件查询, 分页 [详情](process/Paginate.md)                                                                                  |
| create              | models.模型名称.Create              | 创建单条记录, 返回新创建记录 ID [详情](process/Create.md)                                                                     |
| update              | models.模型名称.Update              | 更新单条记录 [详情](process/Update.md)                                                                                        |
| save                | models.模型名称.Save                | 保存单条记录, 不存在创建记录, 存在更新记录, 返回记录 ID [详情](process/Save.md)                                               |
| delete              | models.模型名称.Delete              | 删除单条记录(标记删除) [详情](process/Delete.md)                                                                              |
| destroy             | models.模型名称.Destroy             | 删除单条记录(真删除) [详情](process/Destroy.md)                                                                               |
| insert              | models.模型名称.Insert              | 插入多条记录, 返回插入行数 [详情](process/Insert.md)                                                                          |
| updatewhere         | models.模型名称.UpdateWhere         | 按条件更新记录, 返回更新行数 [详情](process/UpdateWhere.md)                                                                   |
| deletewhere         | models.模型名称.DeleteWhere         | 按条件删除数据, 返回删除行数(标记删除) [详情](process/DeleteWhere.md)                                                         |
| destroywhere        | models.模型名称.DestroyWhere        | 按条件删除数据, 返回删除行数(真删除) [详情](process/DestroyWhere.md)                                                          |
| eachsave            | models.模型名称.EachSave            | 保存多条记录, 不存在创建记录, 存在更新记录, 返回记录 ID 集合 [详情](process/EachSave.md)                                      |
| eachsaveAfterDelete | models.模型名称.EachSaveAfterDelete | 删除一组给定 ID 的记录后，保存多条记录, 不存在创建记录, 存在更新记录, 返回记录 ID 集合 [详情](process/EachSaveAfterDelete.md) |

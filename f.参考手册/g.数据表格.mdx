...
数据表格文档，由表格基础信息、数据模型绑定、业务结构映射表、字段映射表、过滤器映射表、列表页、编辑页、批量插入页和详情页构成。[查看完整示例](#5-完整示例)
...
| 字段    | 类型                         | 说明                                                                                                                                                                          | 必填项 |
| ------- | ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| name    | String                       | 数据表格名称 [查看完整说明](#21-基础信息)                                                                                                                                     | 是     |
| version | String                       | 版本号，用于依赖关系校验和开发平台呈现                                                                                                                                        | 是     |
| bind    | Object Bind                  | 绑定数据模型。 [查看完整说明](#22-绑定数据模型-bind)                                                                                                                          | 否     |
| hooks   | \[key:String\]:String        | 数据表格 Hook                                                                                                                                                                 | 否     |
| apis    | \[key:String\]:Object API    | 数据管理 API 设置。如设置绑定数据模型，自动根据模型信息生成数据管理 API， 可通过设置同 `key` 的配置信息，覆盖自动生成的 API。 [查看完整说明](#23-数据表格管理接口-apis)       | 否     |
| columns | \[key:String\]:Object Column | 字段呈现方式设置。 如设置绑定数据模型，自动根据字段类型信息设置关联组件，可通过设置同 `key` 的配置信息，覆盖自动生成的关联组件信息。 [查看完整说明](#24-字段呈现方式-columns) | 否     |
| filters | \[key:String\]:Object Filter | 查询过滤器设置。 如设置绑定数据模型，自动根据索引和字段信息设置关联组件，可通过设置同 `key` 的配置信息，覆盖自动生成的关联组件信息。 [查看完整说明](#25-查询过滤器-fliters)   | 否     |
| list    | Object Page                  | 列表页设置。 [查看完整说明](#26-列表页-list)                                                                                                                                  | 是     |
| edit    | Object Page                  | 编辑页设置。 [查看完整说明](#27-编辑页-edit)                                                                                                                                  | 是     |
| view    | Object Page                  | 查看页设置。 [查看完整说明](#28-查看页-view)                                                                                                                                  | 否     |
| insert  | Object Page                  | 批量录入页设置。 [查看完整说明](#29-批量录入页-insert)                                                                                                                        | 否     |
...
| withs | \[key:String\]:Object With | 数据模型关联查询设置. [查看数据结构](../model/README.md#31-数据结构) | 否     |
...
| --------------------- | -------- | ------------------------- | ------------------------------------------------------------------------------------------------------------ |
| search                | `GET`    | `/search`                 | 按条件查询, 分页。 [查看完整说明](apis/search.md)                                                            |
| find                  | `GET`    | `/find`                   | 按主键查询单条记录。 [查看完整说明](apis/find.md)                                                            |
| save                  | `POST`   | `/save`                   | 保存单条记录，存在更新，不存在创建。 [查看完整说明](apis/save.md)                                            |
| delete                | `POST`   | `/delete`                 | 按主键删除单条记录。 [查看完整说明](apis/delete.md)                                                          |
| insert                | `POST`   | `/insert`                 | 批量新增记录。 [查看完整说明](apis/insert.md)                                                                |
| delete-where          | `POST`   | `/delete/where`           | 批量删除符合条件的记录。 [查看完整说明](apis/delete-where.md)                                                |
| delete-in             | `POST`   | `/delete/in`              | 批量删除指定一组主键的的数据记录。 [查看完整说明](apis/delete-in.md)                                         |
| update-where          | `POST`   | `/update/where`           | 批量更新符合条件的记录。 [查看完整说明](apis/update-where.md)                                                |
| update-in             | `POST`   | `/update/in`              | 批量更新指定一组主键的的数据记录。 [查看完整说明](apis/update-in.md)                                         |
| import-upload         | `POST`   | `/import/upload`          | 上传文件 `csv`, `xls`, `xlsx` 格式文件，支持大文件分段上传。 [查看完整说明](apis/import-upload.md)           |
| import-preview        | `GET`    | `/import/preview`         | 预览已上传的 `csv` 或 `Excel` 文档结构，用于在 WEB 界面设置字段映射表 [查看完整说明](apis/import-preview.md) |
| import-sync           | `POST`   | `/import/sync`            | 根据字段映射表，同步导入 `csv` 或 `Excel` 数据 [查看完整说明](apis/import-sync.md)                           |
| import-async          | `POST`   | `/import/async`           | 根据字段映射表，异步导入 `csv` 或 `Excel` 数据 [查看完整说明](apis/import-async.md)                          |
| import-async-tasks    | `GET`    | `/import/async/tasks`     | 异步导入任务列表 [查看完整说明](apis/import-async-tasks.md)                                                  |
| import-async-tasks-id | `GET`    | `/import/async/tasks/:id` | 异步导入任务详情 [查看完整说明](apis/import-async-tasks-id.md)                                               |
| quicksave             | `POST`   | `/quicksave`              | 保存多条记录，存在更新，不存在创建。 [查看完整说明](apis/quicksave.md)                                       |
| setting               | `GET`    | `/setting`                | 读取数据表格配置信息, 用于前端界面渲染 [查看完整说明](apis/setting.md)                                       |
...
| view  | Object Render | 列表或查看页面，阅读状态时的使用的组件及属性, 默认为 `label` [查看 Object Render 数据结构](#3-Object-Render-数据结构)  | 否     |
| edit  | Object Render | 列表或查看页面，编辑状态时的使用的组件及属性 , 默认为 `input` [查看 Object Render 数据结构](#3-Object-Render-数据结构) | 否     |
| form  | Object Render | 编辑页面，使用的组件及属性，默认为 `input` [查看 Object Render 数据结构](#3-Object-Render-数据结构)                    | 否     |
...
| input | Object Render | 查询内容输入组件, 默认为 `input` [查看 Object Render 数据结构](#3-Object-Render-数据结构)    | 否     |

### 2.6 列表页 `list`

列表页设置数据结构为 [Object Page 数据结构](#4-Object-Page-数据结构)。
...
| `columns`     | Array\<Object ColumnInstance \| String \> | 数据列表可呈现的列 [ColumnInstance 数据结构](#Object-ColumnInstance-数据结构) | 是     |
...
| actions | \[key:String\]:\[key:String\]:Object Render | 页面可用功能呈现方式映射表，每个页面不同，在具体页面中定义。 [Render 数据结构](#3-Object-Render-数据结构) | 否     |
| option  | \[key:String\]:Any                          | 页面配置信息，根据需要自由定义。API Setting 原样返回。                                                    | 否     |

## 5. 完整示例

完整示例保存在 examples 目录，[查看完整示例](examples)

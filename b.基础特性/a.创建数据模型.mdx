# 创建数据模型

<blockquote>
  一个数据模型对应数据库中的一张数据表, 通过 JSON 文件描述数据表结构，放置在
  models 文件夹中。使用 yao migrate 命令创建/更新数据表结构设计。
</blockquote>

## 数据模型描述

编写一个数据模型描述文件 `pet.mod.json`，并放置在应用的 `models` 文件夹中。

描述文件内容:

```jsonc
{
  "name": "客户",
  "table": { "name": "pet", "comment": "客户表" },
  "columns": [
    { "label": "ID", "name": "id", "type": "ID", "comment": "ID" },
    { "label": "编号", "name": "sn", "type": "string", "unique": true },
    { "label": "名称", "name": "name", "type": "string", "index": true },
    { "label": "介绍", "name": "desc", "type": "string", "comment": "详细介绍" }
  ],
  "values": [
    { "sn": "100001", "name": "Cookie", "desc": "一只猫" },
    { "sn": "100002", "name": "Beibei", "desc": "一只狗" }
  ]
}
```

应用目录结构:

```bash
├── models        # 用于存放数据模型描述文件
│   └── pet.mod.json
├── db
└── ui
└── app.json
```

扩展阅读:

<Extend
  title="模型文档"
  desc="数据模型完整文档"
  link="e.API参考/a.数据模型"
></Extend>

## Migrate 创建/更新数据表

在项目根目录下运行 `Migrate` ，创建/更新数据表结构，并插入默认数据。

```bash
cd /data/app
yao migrate -n pet
```

<Notice type="warning">
  注意：migrate 命令将删除数据库中同名数据表后重新创建，请注意数据备份。
</Notice>

扩展阅读:

<Extend
  title="命令行工具"
  desc="命令行工具完整文档"
  link="e.API参考/a.命令行工具"
></Extend>

## 数据查询

列表查询

```bash
yao run models.pet.Get '::{}'
```

按主键查询数据

```bash
yao run models.pet.Find 1
```

删除一条数据

```bash
yao run models.pet.Delete 1
```

新增一条数据

```bash
yao run models.pet.Create '::{"name":"球球", "desc":"新成员"}'
```

更新一条数据

```bash
yao run models.pet.Update 1 '::{"desc":"一只可爱的三色猫"}'
```

保存一条数据，指定主键则更新，不指定创建创建。

```bash
yao run models.pet.Save '::{"name":"天狼", "desc":"新成员"}'
```

```bash
yao run models.pet.Save '::{"id":1,"desc":"一只可爱的三色猫"}'
```

扩展阅读:

<Extend
  title="模型处理器文档"
  desc="数据模型处理器详细文档"
  link="e.处理器参考/a.数据模型"
></Extend>

## 推荐阅读

接下来，建议学习以下章节:

<Extend
  title="编写接口"
  desc="了解如何编写 RESTFul 数据接口"
  link="b.基础特性/a.编写接口"
></Extend>

<Div style={{ display: "flex", justifyContent: "space-between" }}>
  <Link type="prev" title="安装调试" link="a.介绍/b.安装调试"></Link>
  <Link type="next" title="编写接口" link="b.基础特性/b.编写接口"></Link>
</Div>

...
3. 文件`.yao`的后缀和格式和`.json`结构是一样的，不影响使用都可以兼容。
...
2. 创建并启动容器: `docker run -d --name yao -v <project root>:/data/app -p 5099:5099 -p 5077:5077 yaoapp/yao:0.10.3-amd64`
...
| 镜像                        | 环境     | 使用场景                                                   |
| --------------------------- | -------- | ---------------------------------------------------------- |
| yaoapp/yao:0.10.3-amd64     | 生产环境 | 适用于云端部署                                             |
| yaoapp/yao:0.10.3-arm64     | 生产环境 | 适用于物联网边缘设备部署                                   |
| yaoapp/yao:0.10.3-arm64-dev | 开发环境 | `arm64` 适用于 MacOS M1, 树莓派, RK 开发板等               |
| yaoapp/yao:0.10.3-amd64-dev | 开发环境 | `amd64` 适用于 MacOS Intel, Windows X86 64 位, Linux 64 位 |
...
2. 创建并启动容器: `docker run -d --name yao -v <project root>:/data/app -p 5099:5099 -p 5077:5077 yaoapp/yao:0.10.3-amd64-dev`
...
| 镜像                            | 环境     | 使用场景                                                   |
| ------------------------------- | -------- | ---------------------------------------------------------- |
| yaoapp/yao:0.10.3-dev-amd64     | 生产环境 | 适用于云端部署                                             |
| yaoapp/yao:0.10.3-dev-arm64     | 生产环境 | 适用于物联网边缘设备部署                                   |
| yaoapp/yao:0.10.3-dev-arm64-dev | 开发环境 | `arm64` 适用于 MacOS M1, 树莓派, RK 开发板等               |
| yaoapp/yao:0.10.3-amd64-dev     | 开发环境 | `amd64` 适用于 MacOS Intel, Windows X86 64 位, Linux 64 位 |
...
| 参数               | 必填 | 说明                                                           |
| ------------------ | ---- | -------------------------------------------------------------- |
| --disable-watching | 否   | 禁止使用 watching 监听 `yao start --disable-watching` 命令启动 |
...
新建模型：`/data/app/models/category.mod.yao`然后执行命令 `yao migrate && yao start`

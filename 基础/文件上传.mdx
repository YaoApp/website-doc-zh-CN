...
```jsonc
{
  "图集": {
    "bind": "images",
    "view": { "type": "Image" }, // 使用 Image 组件显示已上传图片
    "edit": {
      "type": "Upload",
      "props": {
        ...
        "$api": {
          ...
          "in": ["$file.file"] // 将 API 参数设定为 $file.file
        }
      }
    }
  }
...
```jsonc
{
  ...
  "nodes": [
    ...
    {
      "name": "文件名",
      "process": "utils.filepath.Join",
      "args": ["texts", "{{$res.当前时刻}}", ".txt"]
    },
    {
      "name": "保存文件",
      "process": "fs.system.WirteFile",
      "args": ["{{$res.文件名}}", "{{$res.文件}}"]
    }
  ],
  "output": "{{$res.文件名}}"
...
function Upload(temp) {
  ...
  var data = fs.ReadTemp(tempFile);
  let now = new Date().getTime();
  let file = `/texts/${now}.txt`;
  fs.system.WirteFile(file, data);
  return file;
}
```
  ...
  "图集": {
    ...
    "edit": {
      ...
      "props": {
        "filetype": "image",
        "$api": {
          "process": "scripts.upload.File", // 绑定 flows.upload.file 处理器
          "in": ["$file.file"]
        }
      }
    }
